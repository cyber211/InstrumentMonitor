...
if(sys.version_info.major == 2):
    ...
    from qwt.qt.QtGui import (QApplication, QPen, QBrush, QFrame,QFont,QToolBar,QStatusBar,
                              ...
                              QWidget,QToolButton, QPrinter, QPrintDialog,QLineEdit,QMessageBox,QComboBox)                          
...
class DataPlot(QwtPlot):
    ...
    def __init__(self, *args):
        ...
        self.setTitle("UUT Reading Monitor - (%s)\r\n"%(self.unit))
    ...
    def setPara(self,y_lower,y_upper,y_interval,x_interval,x_upper,x_lower): # y_lower,y_upper,y_interval,x_interval,X-Upper,x_lower
        if abs(y_upper - self.y_range_Upper) > 0.004:
            self.y_range_Upper = y_upper
        if abs(y_lower - self.y_range_Lower) > 0.004:
    ...
    def StopTimer(self):
        if self.timerId:            
            if(QMessageBox.Yes == QMessageBox.warning(self, 'Warning','Do you want to stop the refresh of the curve?',QMessageBox.Yes | QMessageBox.No,QMessageBox.No)):
    ...
    def Connect(self,COMPort,baudrate):
        if self.uut_dev is None:
            try:
                self.uut_dev = BUNDdevice_py3.SerialDevice(False, False, port = COMPort , baudrate = baudrate) #'''args[0]'''            
        ...
        # Get First reading and init self.z
        tfdata,vfdata = self.uut_get_val(self.uut_dev, config.GetReadingCmd + "\r\n")
        listR = []
        for i in range(len(self.x)):
            listR.append(tfdata)
            self.z = np.array(listR) 
    ...
    def timerEvent(self, e):
        ...
        self.refeshpoint = self.refeshpoint + 1
        
        if self.isFirstReading:
            self.isFirstReading = False            
            self.LastReadingTime = stamp_time
        delta = stamp_time - self.LastReadingTime
        self.LastReadingTime = stamp_time
        ...
        self.setPara(np.amin(self.z) - 0.004,np.amax(self.z) + 0.004 ,(self.y_range_Upper-self.y_range_Lower)/30,self.x_interval,self.x_range,self.x_ZERO)
        ...
        self.SaveData(self.RawReading,vfdata,(delta.seconds*1000 + delta.microseconds/1000))        
    ...
    def uut_get_val(self,uut_dev_in, type = "x?\r\n"):      #[0-9]\d*\.\d+$    #r'[ -+]\d+\.\d+[Ee][-+][0-9]{2}'   #r'[-]?\d+\.\d+' 匹配正负小数
        ...
        #print(cmd_encode)
        #print(config.rsp_regular)
        
        m, m2 = uut_dev_in.trx(cmd_encode,  config.rsp_regular)   #r'^[-]?([0-9]{1,}[.]?[0-9]*)')   # ^[-]?([0-9]{1,}[.]?[0-9]*)$   - 匹配正负小数和整数
        # self.unit = m2
        self.RawReading = m  # for record
        self.unit = 'mOhm'
        
        if m:
            if ('A' in m.upper()) or ('B' in m.upper()) or ('C' in m.upper()) or ('D' in m.upper()): # for BUND 异常，0.3332mohm
                print(m,float(m[:-1]))
                return float(m[:-1])*1000,m2  
            else:
                return float(m)*1000,m2    # for plotting
            
        else:
            return 7200.0,999999.0   ## 
    ...
    def SaveData(self,tfdata,vfdata,delta_ms):
        fh = open(self.fileNamme, "a")        
        now = datetime.datetime.now()
        #timestamp = "%04d-%02d-%02d %02d:%02d:%02d.%03d" % (now.year,now.month,now.day,now.hour, now.minute, now.second, now.microsecond /1000)
        timestamp = now.strftime('%Y-%m-%d %H:%M:%S.%f')
        rstr = "%s, %s, %s, %s,%f\n" % (self.idx,timestamp, tfdata,vfdata,delta_ms)
        fh.write(rstr)
        fh.flush()
...
class CurveDemo(QMainWindow):
    ...
    def __init__(self, *args):
        ...
        if len(port_list) <= 0:
            ...
            for i in range(len(port_list)):                
                COM_LIST.append(list(port_list[i])[0]) # list(port_list[0]) = ['COM10', 'USB Serial Port (COM10)', 'USB VID:PID=0403:6001 SER=AB0JY8QQA']
    ...
    def SetParas(self):
        ...
        '''       
        self.plot.setPara(float(self.LineEdit_y_Lower.text()),
                          float(self.LineEdit_Y_Upper.text()),
                          float(self.LineEdit_y_interval.text()),
    ...
    def GetComSettings_Connect(self):
        '''
            com, baud
        ''' 
        if self.lineEdit_COM != None:
            self.Com_Text = self.lineEdit_COM.text()
        else:
            self.Com_Text = self.comboBox_COM.currentText()
            
        self.showInfo('COM:%s, BaudRate: %d'%(self.Com_Text,int(self.lineEdit_baud.text())))        
        self.plot.Connect(self.Com_Text,int(self.lineEdit_baud.text()))
    ...
    def closeEvent(self, *args, **kwargs):        
        if QMessageBox.Yes == QMessageBox.warning(self, 'Warning','The default Parameters are stored in conf.ini files!',QMessageBox.Yes|QMessageBox.No,QMessageBox.Yes):
           ...
           config.write_item_value('COM','Port',self.Com_Text)
