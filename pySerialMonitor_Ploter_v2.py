...
from qwt import (QwtPlot,QwtPlotMarker,QwtSymbol,QwtLegend,QwtPlotCurve,QwtPlotGrid,QwtPlotCanvas,
                 QwtAbstractScaleDraw,QwtText,QwtPlotRenderer)
...
class DataPlot(QwtPlot):
    ...
    def setPara(self,y_lower,y_upper,y_interval,x_interval):
    ...
    def timerEvent(self, e):
        ...
        tfdata = self.uut_get_val(self.uut_dev, self.getReadingCommand)   # pass/pac
    ...
    def uut_get_val(self,uut_dev_in, type = "x?\r\n"):      #[0-9]\d*\.\d+$    #r'[ -+]\d+\.\d+[Ee][-+][0-9]{2}'   #r'[-]?\d+\.\d+' 匹配正负小数
        ...
        #m, self.unit = uut_dev_in.trx(cmd_encode,  self.getResp_rex)
        #print m.group(0);
        if m:
